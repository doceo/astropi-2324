...
def create_csv(data_file):
    with open(data_file, "w", newline='') as f:
        writer = csv.writer(f)
        header = ("image-1", "image-2", "image_3", "t-im1", "t-im2", "t-im3", "t-diff", "coord-1", "coord-2", "coord-3", "dist", "vel" )
...
'''
DESCRITTORI E MATCHES
'''
def calculate_features(crop_image_1, crop_image_2, crop_image_3, feature_number):
    orb = cv2.ORB_create(nfeatures = int(feature_number))
    ...
    keypoints_3, descriptors_3 = orb.detectAndCompute(crop_image_3, None)

    return keypoints_1, keypoints_2, keypoints_3, descriptors_1, descriptors_2, descriptors_3

def calculate_matches(descriptors_1, descriptors_2, descriptors_3):
    brute_force = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=True)
    
    try:
        matches_12 = brute_force.match(descriptors_1, descriptors_2)
        matches_23 = brute_force.match(descriptors_2, descriptors_3)
        
        matches_12 = sorted(matches_12, key=lambda x: x.distance)
        matches_23 = sorted(matches_23, key=lambda x: x.distance)
        matches = [matches_12, matches_23]
...
'''
def display_matches(image_1_cv, keypoints_1, image_2_cv, keypoints_2, image_3_cv, keypoints_3, matches, count):
    
    # traccia delle linee tra i punti chiave in cui i descrittori corrispondono.
    match_img_1 = cv2.drawMatches(image_1_cv, keypoints_1, image_2_cv, keypoints_2, matches[0][:300], None)
    
    match_img_2 = cv2.drawMatches(match_img_1, keypoints_2, image_3_cv, keypoints_3, matches[1][:300], None)
    ...
    resize_1 = cv2.resize(match_img_1, (1600,600), interpolation = cv2.INTER_AREA)
    resize = cv2.resize(match_img_2, (1600,600), interpolation = cv2.INTER_AREA)
    #cv2.imshow('matches-2-3', resize)
    #resize = cv2.hconcat([resize_1, resize_2])
...
def find_matching_coordinates(keypoints_1, keypoints_2, keypoints_3, matches):
    # tre elenchi vuoti per memorizzare le coordinate di ciascuna caratteristica corrispondente in ciascuna delle immagini.
    coordinates_1 = []
    coordinates_2 = []
    coordinates_3 = []
    ...
    for match in matches:
        image_1_idx = match[0].queryIdx
        image_2_idx = match[0].trainIdx
        image_3_idx = match[1].trainIdx
        
        coordinates = []
        if (image_2_idx == match[1].queryIdx):
            ...
            coordintates.append(coordinates_1)
            coordintates.append(coordinates_2)
            coordintates.append(coordinates_3)
