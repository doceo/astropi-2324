...
def main_functions():
    ...
    data_file = path.join(base_folder, "data-2.csv")
    ...
    for i in range(len(jpg_files)):
        for j in range(i + 1, len(jpg_files)):
            ...
            row.append(jpg_files[i][4:])
            row.append(jpg_files[j][4:])
            image_1 = jpg_files[i]
            image_2 = jpg_files[j]
            ...
            if (date_equal(get_time(image_1), get_time(image_2))):
                ...
                if len(matches)!= 0: 
                    display_matches(crop_image_1, keypoints_1, crop_image_2, keypoints_2, matches, count) # Display matches
